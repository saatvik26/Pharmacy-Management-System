...
def drug_update(Duse, Did, D_Name):
    c.execute(''' UPDATE Drugs SET D_Use = ? WHERE D_id = ?''', (Duse,Did,D_name))
    conn.commit()
def drug_delete(Did):
...
def admin():
    ...
    if choice == "Drugs":
        ...
        if choice == 'Update':
            ...
            d_name= st.text_area("Drug name")
            if st.button(label='Update'):
                drug_update(d_use,d_id,d_name)
    ...
    ## CUSTOMERS
        ...
        st.subheader("By Saatvik Goel (101905094), Yashika Bansal (101955002) , T.Pranav (102085005)")
